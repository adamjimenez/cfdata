...
$html = file_get_contents($url.'/ProjectSearch.aspx');
//print $html; exit;
...
for($c = 0; $c<$node->length; $c++){
    $value = $node->item($c)->getAttribute('data-id');
    //print $value."\n";

    if( preg_match('/ProjectDetails.aspx/', $value, $matches) ){
		//print $url.$value."\n"; exit;
